...
### Overview
1. Introduce a new `.bicepparam` function named `readInput`.

1. Introduce a "parameterInputs" field to the JSON parameters file.

1. Introduce a "parameterInputs" property to the Deployments API, such that the above information can be captured and sent on the API.
...
The proposed `readInput` function has the following structure:
```
readInput(<input_name>[, <input_config>])
```
...
To support this feature, we must introduce a new property "parameterInputs" to the JSON parameters file schema. The structure of this property is intentionally simple for external tools to parse and understand.
...
"parameterInputs". This will exactly match the structure defined above in the parameters file.

"parameterInputValues". This purely exists to contain the actual values of the inputs. Because of the potential for returning secure data, this field will only be supported on a PUT, and will be stripped out of the GET response.

The schema of this new field can be described in TypeScript as:
```ts
type parameterInputValues = {
  [name: string]: any,
};
```
...
I believe this functionality could also be extended to access runtime information like `environment()` or `tenant()`, which are popular asks. However, for this proposal, I'm only planning on implementing the "built-in" expression functions as well as `readInput`.

### Putting it all together

1. User asks tool to deploy a `.bicepparam` file containing `readInput` functions.
1. Bicep converts the `.bicepparam` file to JSON, retaining the configured inputs in the "parameterInputs" section.
1. The tool reads the "parameterInputs" file and processes all of the requested inputs - providing errors if it doen't know how to reolve them.
1. The tool builds the "parameterInputValues" portion of the requet containing the resolved values of the inputs. It sends the template, parameters, parameterInputs and parameterInputValues property to the Deployments API.
1. The Deployments API evaluates language expressions for each parameter with an "expression" property defined, using the parameterInputValues property to handle the `readInput` function evaluation.
...
param foo = readInput('sys.envVar', 'MY_FOO_VAR')
```

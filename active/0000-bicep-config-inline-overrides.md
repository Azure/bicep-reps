...
1. Should we implement the more complex JSON-PATCH (RFC 6902) or directive-based override syntax _initially_ or as follow-up? These would likely take time to iron out the design and potentially delay the Microsoft-internal tooling authoring design
2. Should malformed JSON in `--config-override` or unrecognized config properties in overrides by cause errors, emit warnings, or ignored?
3. Should we include a `--show-effective-config` flag or similar to help users troubleshoot what configuration is actually being applied after overrides?
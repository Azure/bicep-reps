...
Resource API candidates use Bicep's existing type catalog and the version lookup behind `use-recent-api-versions`, not a new Azure service. That rule is off by default; its default `maxAgeInDays` is 730 and `gracePeriodInDays` is 90. It distinguishes all known versions from versions acceptable to recommend, considering version age and stable versus preview status. Extract and reuse that lookup and recommendation logic without changing linter behavior. `outdated` reports known newer versions even when the rule is disabled or the current version passes its age policy; recommendation context must remain distinguishable from availability. Whether Dependabot's separately enabled resource ecosystem uses the linter's recommendation policy to select PRs needs an explicit decision. The catalog is not proof of availability in a particular subscription or region, and an API-version date is not necessarily the publication date required for Dependabot cooldowns. Date-based API versions must not be compared as SemVer.
...
2. Add Bicep graph-aware inventory, registry and API-version candidate discovery, versioned `outdated` output, and exact `update` edits. Extract applicable linter lookup, comparison, and edit logic while preserving existing linter results. Start end-to-end validation with public AVM modules; keep resource and CLI-pin kinds explicit rather than exposing them through the module job.
...
- Which module reference forms and resource API expressions are supported first? Should resource API PRs use the linter's age, grace-period, and stable/preview recommendation policy or a separately agreed rule?

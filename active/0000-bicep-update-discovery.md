...
`module`, `recommended`, and `human` are the defaults; the other kinds are explicit so adding them cannot change module-only results. `--schema-version` requires JSON and defaults to 1. Dependabot requests exactly one kind. `--selection recommended` proposes only versions Bicep would suggest for the current reference; `--selection available` opens the broader pool of comparable newer candidates (plus any linter-suggested stable replacement of a preview). Both people and Dependabot can explicitly request `available`. Existing `bicepconfig.json` settings for the resource linter's maximum age and grace period still shape recommendations, even if the diagnostic is disabled. The CLI selection does not change the linter's policy or claim that every available version is safe.
...
The existing [`allow` and `ignore` options](https://docs.github.com/en/code-security/reference/supply-chain-security/dependabot-options-reference#allow--) filter dependencies, version ranges, or SemVer update types; [`cooldown`](https://docs.github.com/en/code-security/reference/supply-chain-security/dependabot-options-reference#cooldown--) delays releases; [`versioning-strategy`](https://docs.github.com/en/code-security/reference/supply-chain-security/dependabot-options-reference#versioning-strategy--) governs how supported ecosystems edit version requirements. Reducing a cooldown can make updates arrive sooner, but none of these settings says to disregard Bicep's API-version age and grace policy. Treating the presence of an `allow` rule as an override would be wrong: Dependabot already considers all declared dependencies by default. We therefore propose one **new**, ecosystem-specific option, subject to Dependabot maintainer and hosted-configuration review:
...
updates:
  - package-ecosystem: "bicep-resources"
    ...
    schedule:
      interval: "weekly"
    bicep-update-selection: "available" # Proposed; not currently a supported dependabot.yml key.
```

Without that opt-in, the adapter calls `bicep outdated --selection recommended` and applies ordinary Dependabot rules to Bicep's suggestions. With it, the adapter calls `--selection available` and applies the **same** `allow`, `ignore`, cooldown, grouping, and applicable prerelease filters to the broader pool; it does not disable those controls. The setting is per ecosystem job, not a new Bicep linter setting. SemVer `update-types` can filter comparable module tags, but must not be applied to date-based resource API versions as though they were SemVer. Whether Dependabot will accept this new configuration key is an explicit review question, not a capability of current `dependabot.yml`.
...
- A new Dependabot selection key makes aggressive updates intentional without repurposing `allow` or weakening `ignore` and cooldown. It requires hosted-configuration support and Dependabot maintainer approval; without it, Dependabot cannot opt into Bicep's broader candidate pool through today's YAML options.
...
1. Confirm ecosystem boundaries and names, the proposed YAML opt-in, reachable-file update behavior, registry-secret delivery, and supported pinned CLI versions with Dependabot maintainers.
...
3. Contribute the module ecosystem to dependabot-core using the native commands and wire the proposed opt-in after Dependabot configuration review. Add registry-proxy, inherited-config, redirect, multiple-occurrence, excluded-file, and stale-edit cases to its packaged-runner tests. Enable it in the hosted service under the agreed preview process; test private registry jobs end to end before claiming support for them.
...
- Will Dependabot accept three ecosystems, the proposed `bicep-update-selection` key, and updates to reachable, non-excluded files outside `directory`?

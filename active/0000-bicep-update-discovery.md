...
```sh
bicep outdated <entrypoint> [--kind module|resource-api|cli] [--selection recommended|all-newer] [--output-format human|json] [--schema-version 1]
...
`module`, `recommended`, and `human` are the defaults; the other kinds are explicit so adding them cannot change module-only results. `--schema-version` requires JSON and defaults to 1. Dependabot always requests exactly one kind and uses `recommended`; it does not turn `allow`, `ignore`, or prerelease settings into a Bicep policy override. `--selection all-newer` is an explicit opt-in for people invoking the CLI: it shows all comparable newer candidates, including those Bicep would not recommend, alongside any linter-suggested same-date stable replacement. It does not make Dependabot more aggressive; offering that through `dependabot.yml` would need a separate design and Dependabot review. Existing `bicepconfig.json` settings for the resource linter's maximum age and grace period still shape the default recommendations, even if the diagnostic is disabled. There is no new update-policy configuration in this REP.

`outdated` makes no source changes. In human mode, show a short table of **Source**, **Dependency**, **Current**, **Recommended**, and **Latest available**; identify the resolved registry for modules. Under `all-newer`, add **Other newer** candidates, clearly marked as outside Bicep's recommendation. For example:
...
For `bicep outdated infra/main.bicep --selection all-newer`, the same row is:
...
The module linter suggests only the newest version. With no recommendations, print `No recommended module updates.` rather than an empty table; under `all-newer`, print `No newer module versions found.` only if the broader candidate set is also empty. For resources, show the effective maximum age and grace period alongside the table. An accepted current API version can have newer available versions without being reported as outdated by default. The resource rule's suggested version may even be a same-date stable version in place of a preview; preserve that recommendation rather than treating "newer date" as the only possible fix. For exact CLI pins, the default recommendation is the newest eligible CDN release; this is a release-selection rule, not a linter rule.

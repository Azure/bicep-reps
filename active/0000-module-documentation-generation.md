...
This REP supersedes the draft proposal originally raised as [Azure/bicep#19694](https://github.com/Azure/bicep/pull/19694).

## Terms and definitions

* **AVM** - [Azure Verified Modules](https://aka.ms/avm), the Microsoft-maintained library of Bicep and Terraform modules that defines the documentation layout this feature aims to be compatible with.
* **`Set-ModuleReadMe.ps1`** - the existing AVM PowerShell tool that parses compiled Bicep output and external metadata to produce a module `README.md`. It is the prior art this feature is intended to replace for most consumers.
* **Module entrypoint** - the `.bicep` file that is intended to be consumed as a module. By Bicep and AVM convention this is `main.bicep` at the module root. Other `.bicep` files in the tree are typically implementation details.
* **Preset** - a named, built-in output configuration selected with `--preset`. The only preset in v1 is `markdown`.
* **Docs model** - the typed, semantic model projected from compiled Bicep information and handed to the template renderer.
* **Template add-in file** - an authored Markdown fragment (for example `_header.md` or `_footer.md`) that a custom template composes into the rendered document through include syntax.
* **Scriban** - the .NET templating engine proposed for rendering. See [Alternatives](#templating-engine-alternatives) for the full evaluation.
* **Golden-file test** - a test that compares generated output byte-for-byte against a checked-in expected file.
* **JSON-RPC** - the existing Bicep JSON-RPC surface (`bicep jsonrpc`) used by long-lived clients to drive the compiler without repeated process startup.
...
Not applicable. This feature operates entirely on locally compiled Bicep and does not require any deployment engine or service-side change.

### Microsoft.Resources/deployments API changes

Not applicable. No changes to the `Microsoft.Resources/deployments` API contract are required.

### Documentation engine and data model
...
Use `Scriban` as the templating engine. See [Alternatives](#templating-engine-alternatives) for a comparison of considered options and a more detailed rationale for the choice.
...
The preferred authoring experience is a direct include-style syntax where the main template references the file name it wants to embed. A worked example is provided in [Examples](#examples).
...
The representative request and response shapes are shown in [Examples](#examples).
...
6. Verify `--output-file` changes the file name but not the rendered content.
7. Verify `--template-file` can render a custom Scriban template.
8. Verify a custom template can consume add-in files such as `_header.md` and `_footer.md`.
9. Verify a custom template can consume caller-supplied `--set key=value` strings.
10. Verify `bicep docs generate` overwrites the entire output file and does not preserve existing content already present in that file.
11. Verify compiler errors prevent output writes and produce the expected non-zero exit code.
12. Verify JSON-RPC generation returns the same content as the CLI services.
13. Verify the docs engine in `Bicep.Core` can be invoked directly without going through CLI-specific types.
...
- Custom template composing add-in files
...
- Representative JSON-RPC request shape
...
- Representative JSON-RPC response shape
...
1. **Choosing a templating engine is a long-lived, hard-to-reverse commitment.** Once module authors ship custom templates, the template syntax and the exposed model become a public compatibility contract. Mitigation: Scriban is sandboxed, actively maintained, Apache-2.0 licensed, and offers a Liquid-compatibility mode for authors from non-.NET backgrounds; the template model is explicitly designed as a versionable public surface; Fluid remains a viable fallback if a Scriban-specific blocker appears before the feature stabilizes.
2. **Placing the docs engine in `Bicep.Core` grows its public surface and adds a third-party dependency to the library every consumer already references.** Mitigation: the engine is kept free of CLI-only abstractions, and co-location avoids exposing `Bicep.Core` internals across a new package boundary. Extracting it into a separate package later would itself be a breaking change for `Bicep.Core` consumers.
3. **`bicep docs generate` always overwrites the entire output file and never merges.** Any hand-authored content in an existing `README.md` is lost on first run. Mitigation: template add-in files (`_header.md`, `_footer.md`) provide a supported place for authored content, `--output-file` allows writing to a different file, and the deterministic whole-file model keeps output reproducible and diffable in source control.
4. **AVM structural compatibility is not byte-for-byte parity.** v1 deliberately omits network-dependent behavior such as live URL validation, so output will differ from `Set-ModuleReadMe.ps1` in places. AVM will need to either accept the new output or retain a thin wrapper for the remaining AVM-specific concerns.
5. **`docs` is the first grouped verb in the CLI**, requiring the command parser and help text to evolve and introducing a surface that is structurally inconsistent with the existing single-verb commands until others follow.

This REP introduces no breaking changes. It does not alter existing `build`, `lint`, `format`, or `jsonrpc` behavior beyond what is required to add the new docs surface.

## Alternatives

### Keep documentation generation in downstream tooling

The status quo is AVM's `Set-ModuleReadMe.ps1`. This works today but leaves documentation generation out-of-tree, keeps it inaccessible to non-AVM module authors, and means Bicep language improvements do not automatically improve documentation output. It also forces every consumer that wants AVM-quality docs to adopt AVM tooling.

### Ship the engine as a separate NuGet package or standalone tool

A dedicated package would give a cleaner boundary, but the engine depends heavily on `Bicep.Core` services (compilation, semantic model, diagnostics, type system) and would require exposing additional internals across a package boundary. It would also add release, versioning, and dependency-management cost for v1 without giving existing consumers anything they cannot already get from `Bicep.Core`.

### Use a single flat verb such as `bicep generate-docs`

This would match the existing single-verb CLI exactly and avoid changing the command parser. It was rejected because documentation generation is expected to grow beyond one operation (`validate`, `init-template`), and retrofitting a command group later would either break the flat verb or leave two inconsistent surfaces.

### Introduce a `--recursive` flag instead of `--pattern`

A `--recursive` flag was considered for bulk generation. `--pattern` was chosen instead because it already exists on `bicep build`, `bicep format`, `bicep lint`, `bicep restore`, and `bicep build-params`, so callers get a familiar surface, and because a glob expresses module-entrypoint selection (`**/main.bicep`) far more precisely than a recursion switch.

### Merge into or preserve sections of an existing README

A merge model would let authors keep hand-written content directly in `README.md`. It was rejected for v1 because it makes output a function of prior output, which is non-deterministic, hard to test with golden files, and difficult to reason about when templates change. Add-in files provide the same authoring capability with a deterministic model.

### Build a custom templating language

Rejected. It would require designing, documenting, securing, and maintaining a language, and would be unfamiliar to every author, for no benefit over a mature sandboxed engine.

### Templating engine alternatives
...
There are no backend or deployment engine changes, so no ARM feature flag is required.

The CLI surface should ship behind an experimental feature flag until the template model and rendered output are proven. This allows the docs model, the built-in template, and the add-in file mechanism to change without breaking users while AVM and early adopters exercise the feature.
...
1. Land the v1 scope behind the experimental flag.
2. Adopt the command in AVM to validate structural compatibility against real modules at scale, and use the results to refine the built-in template.
3. Confirm the docs model and template contract are stable enough to be treated as a supported public surface.
4. Remove the experimental flag and move this REP to final.
...
Additionally, to be resolved through the REP process before merging:

4. Whether Scriban's native syntax is sufficiently familiar to module authors coming from non-.NET backgrounds, or whether the built-in template and documentation should standardize on Scriban's Liquid-compatibility mode to broaden familiarity.

To be resolved through implementation before stabilization:

5. The precise include resolution and add-in file mechanism, including how `--template-root` interacts with includes that traverse outside the module root.
6. The exact repository-local discovery rules for usage examples, and how closely they should match AVM's existing test file conventions.

## Out of scope

The following are explicitly not addressed by this REP, but could be addressed independently in the future:
...
5. Additional `bicep docs` subcommands such as `bicep docs validate` and `bicep docs init-template`. The command group is designed to accommodate them, but they are not part of this REP.
6. Richer typed (non-string) values for `--set`. If needed later, this can be added as a separate extension to the templating model.
7. Merge or section-preservation semantics for an existing output file.
